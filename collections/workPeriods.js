...
Meteor.methods({
  'setWorkPeriod': function(workPeriod) {
    check(workPeriod,{
      ...
      todoID: Match.Optional(Match.OneOf(Match.idString,null)),
    ...
    if (!('activityID' in workPeriod) && !('todoID' in workPeriod))
      throw new Meteor.Error('activityOrTodo','A workperiod must refer to an activity or a todo item.')
    if (('activityID' in workPeriod) && (workPeriod.activityID)) {
      ...
      var activity = null;
      workPeriod.activityID = null;
      workPeriod.unitID = null;
      workPeriod.activityVisible = null;
    }
    if (('todoID' in workPeriod) && (workPeriod.todoID)) {
      var todo = Todos.findOne(workPeriod.todoID);
      if (!todo)
        throw new Meteor.Error('todo-item-not-found', "Cannot set work period.  Could not find todo item " + workPeriod.todoID + '.');
      if (!Todos.check.userCanEdit(workPeriod.todoID,cU))
        throw new Meteor.Error('noEditingPrivileges',"User" + userID + " does not have permission to set work period for todo item" + workPeriod.todoID + ".");
    } else {
      var todo = null;
      workPeriod.todoID = null;
    }
    ...
    Sections.find(selector).forEach(function(section) {
      var wP = WorkPeriods.findOne({
        sectionID:section._id,
        activityID:workPeriod.activityID,
        todoID:workPeriod.todoID
      });
      if (wP) {
  ...
  'deleteWorkPeriod': function(workPeriod) {
    ...
    if (wP.todoID) {
      ...
      if (!Todos.check.userCanEdit(workPeriod.todoID,cU))
        throw new Meteor.Error('noEditingPrivileges',"User" + userID + " does not have permission to set work period for todo item" + workPeriod.todoID + ".");
      if (wP.activityID)
        throw new Meteor.Error('linkedToActivity',"Cannot delete todo items that are linked to an activity, as they probably set the deadline for an assessment.");
    }
    if (wP.activityID) {
      if (!Roles.userIsInRole(cU,'teacher'))
        throw new Meteor.Error('notTeacher','Only a teacher can set or change a work period.');
    ...
    } else { //workPeriod only linked with todo item
      var selector =  (workPeriod.sectionID == 'applyToAll') ? {} : {_id:workPeriod.sectionID};
      Sections.find(selector).forEach(function(section) {
        var wP = WorkPeriods.findOne({sectionID:section._id,todoID:workPeriod.todoID});
        if (wP) {
          WorkPeriods.remove(wP._id);
        }
      });
